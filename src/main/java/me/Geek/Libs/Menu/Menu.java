...
public final class Menu {
    ...
    // 缓存的菜单页面 键 = 菜单唤起指令
    private final Map<String, MTag> MenuCache = new HashMap<>();
    ...
    public Inventory Build(Player player, String MenuTag) {
        MTag tag = MenuCache.get(MenuTag);
    ...
    public MTag getMenuTag(String MenuID) {
        return this.MenuCache.get(MenuID);
    }

    // 通过菜单绑定的命令，获取该菜单的名字，菜单ID
    ...
    private ItemStack onBuildItem(String iconID, List<Micon> miconObj) {
            for (Micon icon : miconObj) {
                if (Objects.equals(icon.getIcon(), iconID)) {
                    ...
                    itemMeta.setDisplayName(icon.getName());
                    itemMeta.setLore(icon.getLore());
