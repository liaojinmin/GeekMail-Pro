...
object HexUtils {
    ...
    private fun translateHex(hex: String): String {
        return if (BukkitVersion >= 1160) ChatColor.of(hex).toString() else translateHex(Color.decode(hex))
    }

    private fun translateHex(color: Color): String {
        if (BukkitVersion >= 1160) return ChatColor.of(color).toString()
    ...
    open class Gradient(colors: List<Color>, steps: Int) :
        ...
        override fun next(): Color {
            ...
            if (BukkitVersion < 1160 || steps <= 1) return gradients[0].colorAt(0)
    ...
    open class Rainbow(totalColors: Int, saturation: Float, brightness: Float) :
        ...
        init {
            var totalColor = totalColors
            if (totalColor < 1) totalColor = 1
            hueStep = 1.0f / totalColor
